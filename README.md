...
- 📰 **注目ニュース** — Yahoo!・Google トレンド・はてブ・Qiita・Zenn・Publickey・Google News から3カテゴリ（一般 / IT・技術 / インフラ・クラウド）を取得。**直近48時間**に絞り込み、**鮮度＋話題度（ブックマーク数など）でランク付け**したうえで Gemini API が日本語要約

### ニュースの選定ロジック

「その日・前日に話題になったトピック」に追従するため、各カテゴリで以下の順に処理します。

1. **収集** — カテゴリごとの複数フィードを並列取得（1つ失敗しても他は継続）
2. **鮮度フィルタ** — 公開から48時間以内の記事だけ残す（`RECENT_WINDOW_HOURS`）
3. **重複排除** — 同一リンクは1件に集約
4. **話題度ランキング** — `鮮度スコア` と `話題度スコア`（はてブのブックマーク数 → 正規化、無い場合はソース種別の代理値）を半々で合成し降順に並べる
5. **AI要約** — 上位を Gemini に渡し、ソース・ブックマーク数・経過時間も添えて「本日・昨日に話題のトピック優先」で5件要約

> 「世界のトレンド」も拾いたい場合は `src/news.ts` の `GOOGLE_TRENDS_FEEDS` に `{ geo: "US", source: "Google Trends(世界)" }` を追加するだけで対象国を増やせます。
